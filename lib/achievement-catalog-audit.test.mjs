...
test('audited semantic fixes cannot regress', () => {
  assert.match(dashboard, /getPirateBattleVictoryProgress\(\{[\s\S]*?historyStats,[\s\S]*?chapter4LandDefenseVictories:[\s\S]*?chapter4SeaDefenseVictories:[\s\S]*?chapter4DirectBattleVictories:/);
