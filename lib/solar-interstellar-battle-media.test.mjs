...
import {
  ...
  isSolarInterstellarBattle,
  pickManualBattleTheme,
} from './solar-interstellar-battle-media.mjs';

test('enables the custom experience for every Solar and Interstellar battle', () => {
  assert.equal(isSolarInterstellarBattle('Solar'), true);
  assert.equal(isSolarInterstellarBattle('Interstellar'), true);
  assert.equal(isSolarInterstellarBattle('Void'), false);
});
...
test('BattleOverlay preserves result handling while delegating every chapter battle', async () => {
  const battleOverlay = await readFile(
    new URL('../components/dashboard/BattleOverlay.tsx', import.meta.url),
    'utf8',
  );

  assert.match(battleOverlay, /isSolarInterstellarBattle/);
...
test('chapter battle transition stays visible while decoded assets become ready', async () => {
  const [arena, assetPreloader] = await Promise.all([
    readFile(new URL('../components/VoidBattleArena.tsx', import.meta.url), 'utf8'),
    readFile(new URL('./asset-preloader.ts', import.meta.url), 'utf8'),
  ]);

  assert.match(arena, /!assetsLoaded && routeTier === 'Void'/);
  assert.match(arena, /!assetsLoaded && routeTier !== 'Void'/);
  assert.match(assetPreloader, /\/assets\/rota1\/battle\/background_battle\.mp4/);
  assert.match(assetPreloader, /\/assets\/rota2\/battle\/background_battle\.mp4/);
  assert.match(assetPreloader, /\/assets\/rota1\/battle\/layer_background1\.webp/);
  assert.match(assetPreloader, /\/assets\/rota2\/battle\/layer_background1\.webp/);
  assert.match(assetPreloader, /\/images\/battle\/solar\/meteorite1\.webp/);
});
