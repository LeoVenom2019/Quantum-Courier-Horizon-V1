...
interface VoidAircraftTabProps {
  // Most props are now consumed via context
}
                    ...
                    {[
                      ...
                      return (
                        <button
                          ...
                          className={`flex flex-col p-3 rounded-lg border transition-all text-left group ${isMaxLevel ? 'bg-emerald-500/10 border-emerald-500/20 cursor-default' : qc >= costValue ? 'bg-white/5 border-white/10 hover:border-cyan-500/50' : 'bg-black/20 border-white/5 opacity-50 cursor-not-allowed'}`}
                        >
                          <div className="flex items-center gap-2 mb-1">
                          ...
                          <div className="flex justify-between items-end">
                            ...
                            {!isMaxLevel && <span className="text-base font-orbitron font-bold text-white">{formatValue(costValue)}</span>}
                          </div>
                        </button>
                      );
                    })}
