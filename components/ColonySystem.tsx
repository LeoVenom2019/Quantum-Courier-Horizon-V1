  ...
  return (
    <div className="flex flex-col h-full space-y-6 overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div>
            <div className="flex items-center gap-2">
               <p className="text-zinc-500 text-xs font-mono uppercase tracking-wider">{t('Active Colony', 'Colônia Ativa')}</p>
               <span className="text-[10px] px-1.5 py-0.5 bg-blue-500/20 text-blue-400 rounded-md font-mono">
                {activeColony.age || 0} {t('Years', 'Anos')}
              </span>
            </div>
            <h3 className="text-xl font-bold text-white">{activeColony.name}</h3>
          </div>
        ...
        >
          <div>
            <p className="text-zinc-500 text-xs font-mono uppercase tracking-wider">{t('Available Constructors', 'Construtores Disponíveis')}</p>
            <h3 className="text-xl font-bold text-blue-400">{availableConstructors} / {activeColony.constructors}</h3>
          </div>
          <Bot className="text-blue-400 opacity-50" size={32} />
        </motion.div>
        ...
        >
          <div>
            <p className="text-zinc-500 text-xs font-mono uppercase tracking-wider">{t('Population', 'População')}</p>
            <div className="flex items-baseline gap-2">
              <h3 className={`text-xl font-bold ${activeColony.population > 0 ? 'text-green-400' : 'text-zinc-500'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-1 overflow-hidden">
        ...
        <div className="lg:col-span-1 bg-zinc-900/30 border border-zinc-800 rounded-xl p-4 flex flex-col space-y-4">
          <h4 className="text-sm font-mono text-zinc-400 uppercase tracking-widest flex items-center gap-2">
            <Users size={16} /> {t('Migration & Growth', 'Migração e Crescimento')}
          </h4>
          
          <div className="flex-1 flex flex-col items-center justify-center text-center p-4 border border-zinc-800/50 rounded-xl bg-zinc-900/20">
             {!activeColony.isHabitable ? (
               <>
                 <div className="p-4 rounded-full bg-red-500/10 text-red-500 mb-4 animate-pulse">
                    <Shield size={48} />
                 </div>
                 <h5 className="text-white font-bold mb-2">{t('Migration Locked', 'Migração Bloqueada')}</h5>
                 ...
                 <div className="mt-6 w-full space-y-2 text-[10px] font-mono text-left">
               ...
               <>
                 ...
                 <h5 className="text-white font-bold mb-2">{t('Migration Open', 'Migração Aberta')}</h5>
                 ...
                 <div className="w-full space-y-2">
                    <button 
                      ...
                      className="w-full py-2 text-[10px] font-bold bg-green-600/20 border border-green-500/50 hover:bg-green-600 hover:text-black text-green-400 rounded transition-all"
                    >
                    ...
                    <button 
                      ...
                      className="w-full py-2 text-[10px] font-bold bg-green-700/30 border border-green-600/50 hover:bg-green-700 hover:text-white text-green-300 rounded transition-all"
                    >
          ...
          <div className="p-3 rounded-lg bg-blue-500/5 border border-blue-500/20">
             <div className="flex items-center gap-2 mb-2 text-blue-400">
                <TrendingUp size={14} />
                <span className="text-[10px] font-bold uppercase">{t('Growth Factor', 'Fator de Crescimento')}</span>
             </div>
        ...
        <div className="lg:col-span-3 bg-zinc-900/10 border border-zinc-800/50 rounded-xl p-6 overflow-y-auto custom-scrollbar">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {activeColony.constructions.map((con) => {
              ...
              const colorClasses: Record<string, string> = {
                emerald: 'border-emerald-500/80 shadow-[0_0_20px_rgba(16,185,129,0.4)]',
                zinc: 'border-zinc-400/80 shadow-[0_0_20px_rgba(161,161,170,0.4)]',
                yellow: 'border-yellow-400/80 shadow-[0_0_20px_rgba(234,179,8,0.4)]',
                blue: 'border-blue-400/80 shadow-[0_0_20px_rgba(59,130,246,0.4)]',
                red: 'border-red-500/80 shadow-[0_0_20px_rgba(239,68,68,0.4)]',
                'restaurant-mix': 'border-l-emerald-400 border-t-emerald-400 border-r-red-400 border-b-red-400 shadow-[0_0_20px_rgba(16,185,129,0.3),0_0_20px_rgba(239,68,68,0.2)]'
              };
              ...
              return (
                <motion.div
                  key={con.id}
                  layout
                  className={`relative p-5 rounded-2xl border-2 bg-black/40 flex flex-col ${colorClasses[config.color]} ${isMaxed ? 'opacity-80' : ''}`}
                >
                  <div className="flex items-center justify-between mb-4">
                    <div className={`p-2.5 rounded-xl ${accentColor[config.color]}`}>
                      <config.icon size={24} />
                    </div>
                    <div className="text-right">
                       <span className="text-[10px] font-mono text-zinc-500 block uppercase">{t('Units Built', 'Unidades')}</span>
                       <span className="text-xl font-michroma text-white">{con.level} / 10</span>
                    </div>
                  </div>

                  <h5 className="font-bold text-white text-base mb-1">{config.label[language]}</h5>
                  <div className="flex items-center gap-2 mb-6">
                     <div className={`h-1 flex-1 rounded-full bg-zinc-800 overflow-hidden`}>
                        <motion.div 
                          initial={{ width: 0 }}
                          animate={{ width: `${isMaxed ? 100 : con.progress}%` }}
                          className={`h-full ${isMaxed ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]' : 'bg-blue-500 animate-[pulse_2s_infinite]'}`}
                        />
                     </div>
                     {!isMaxed && (
                       <span className="text-[10px] font-mono text-zinc-500 w-8">{Math.floor(con.progress)}%</span>
                     )}
                  </div>
                    ...
                    {!isMaxed ? (
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center justify-between text-[10px] font-mono text-zinc-400 px-1">
                           <span>{t('Robots Assigned:', 'Robôs Alocados:')}</span>
                        ...
                        <div className="flex gap-1.5">
                           <button 
                             ...
                             className="flex-1 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-500 transition-colors disabled:opacity-20"
                           >
                              -50
                           </button>
                           <button 
                             onClick={() => updateConstructors(con.id, 50)}
                             className="flex-1 py-1.5 rounded-lg bg-blue-600/20 border border-blue-500/40 text-blue-400 hover:bg-blue-600 hover:text-white font-bold transition-all"
                           >
                              +50
                           </button>
                           <button 
                             onClick={() => updateConstructors(con.id, 250)}
                             className="flex-1 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-400 font-bold transition-colors"
                           >
                              +250
                           </button>
                        </div>
                      ...
                      <div className="flex items-center justify-center p-2 rounded-xl bg-green-500/10 border border-green-500/20 text-green-500 text-[10px] font-mono uppercase tracking-widest gap-2">
                         <CheckCircle2 size={12} />
                         {t('Category Maxed', 'Categoria Completa')}
                      </div>
                    )}
                  </div>
